...
$id = $_GET["id"];
$koneksi = koneksiUts();
$sql = "SELECT * FROM item_home WHERE id=". $id;

$data = mysqli_query($koneksi,$sql);

if(isset($_POST["ubah"])){
    if(updateItemHome($_POST) > 0){
        echo"<script>
        alert('data berhasil di ubah !');
        document.location.href='item-home.php';
        </script>";
    }
    else{
        echo "<script>
